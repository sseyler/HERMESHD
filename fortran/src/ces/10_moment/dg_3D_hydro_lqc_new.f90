...
!   Finally in comparing the cases (3,27) to (4,20) there does not appear to be any real differences, only that the (4,20) case was about 15% faster.
	...
	integer, parameter :: nx = 40, ny = 40, nz = 1, ngu = 0, nbasis = 8, nbastot = 30
	...
	real, parameter :: aindm1 = aindex - 1.0, cp = aindex/(aindex - 1.0), clt = 2.
    	...
    	! derived units 
        real, parameter :: v0=L0/t0, p0=mu*1.67e-27*n0*v0**2, te0=p0/n0/1.6e-19, vis=1.e-4
...
!        Pres = (aindex - 1.)*(Qin(en) - 0.5*dn*(vx**2 + vy**2 + vz**2))
        Pres = (Qin(exx) + Qin(eyy) + Qin(ezz) - dn*(vx**2 + vy**2 + vz**2))/3.
        Tev = Pres*dni
...
!----------------------------------------------------------------------------------------------
    ...
    implicit none
    integer i,j,k,ieq,ipg,ir
    real, dimension(nx,ny,nz,nQ,nbasis) :: Q_ri,Q_rip 
    real Qout(npg,nQ)	
    real t,source(npg,nQ),Qin(nQ),dn,dni,Zin,vx,vy,vz,alpha,temp,dne,eta_a,Teev,Tiev,etaJ2,nuei,TemR,Tev,vmax
	...
	do ieq=exx,nQ	
	   do ir=1,nbasis
	      Q_rip(i,j,k,ieq,ir) = 0.125*cbasis(ir)*sum(wgt3d(1:npg)*bfvals_int(1:npg,ir)*Qout(1:npg,ieq))
	   end do
	end do
...
!	fpnts_r(ife,eyz) = vz*(dn*vy**2 + Pyy) + 2*vy*Pyz                            ! term 17
	...
	fpnts_r(ife,en) = (Qpnts_r(ife,en) + Pzz)*vz + vx*Pxz + vy*Pyz 
...
!	fpnts_r(ife,exx) = vz*(dn*vx**2 + Pxx) + 2*vx*Pxz                              ! term 3
!	fpnts_r(ife,eyy) = vz*(vy**2 + Pyy) + 2*vy*Pyz                                 ! term 6
!	fpnts_r(ife,ezz) = vz*(dn*vz**2 + 3*Pzz)                                       ! term 9
!
!	fpnts_r(ife,exy) = dn*vx*vy*vz + vx*Pyz + vy*Pxz + vz*Pxy                      ! term 12
!	fpnts_r(ife,exz) = vx*(dn*vz**2 + Pzz) + 2*vz*Pxz                              ! term 15
!	fpnts_r(ife,eyz) = vy*(dn*vz**2 + Pzz) + 2*vz*Pyz                              ! term 18
