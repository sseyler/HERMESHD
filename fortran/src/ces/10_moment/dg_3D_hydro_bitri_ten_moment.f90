...
!-------------------------------------------------------------------------------------------------------------------------------------------                                        
	...
	integer, parameter :: nx = 40, ny = 40, nz = 1, ngu = 0, nbasis = 8, nbastot = 35
...
!----------------------------------------------------------------------------------------------
	...
	do ir=1,nbasis
        ...
        Q_rp(i,j,k,exx,ir) = (Q_ri(i,j,k,exx,ir) + dt*source_r(i,j,k,exx,ir)                                                               &
        ...
        Q_rp(i,j,k,eyy,ir) = (Q_ri(i,j,k,eyy,ir) + dt*source_r(i,j,k,eyy,ir)                                                               &
                           + coll*dt*(Q_ri(i,j,k,exx,ir) + Q_ri(i,j,k,eyy,ir) + Q_ri(i,j,k,ezz,ir))*oth                                    &
                           + coll*dt**2*(source_r(i,j,k,exx,ir) + source_r(i,j,k,eyy,ir) + source_r(i,j,k,ezz,ir))*oth)/(1. + dt*coll)

        Q_rp(i,j,k,ezz,ir) = (Q_ri(i,j,k,ezz,ir) + dt*source_r(i,j,k,ezz,ir)                                                               &
                           + coll*dt*(Q_ri(i,j,k,exx,ir) + Q_ri(i,j,k,eyy,ir) + Q_ri(i,j,k,ezz,ir))*oth                                    &
                           + coll*dt**2*(source_r(i,j,k,exx,ir) + source_r(i,j,k,eyy,ir) + source_r(i,j,k,ezz,ir))*oth)/(1. + dt*coll)
	...
	do ieq = exy,nQ
	  do ir=1,nbasis
	    Q_rp(i,j,k,ieq,ir) = (Q_ri(i,j,k,ieq,ir) + dt*source_r(i,j,k,ieq,ir))/(1. + dt*coll)
	  end do
	end do
...
!-----------------------------------------------------------!        
   real function cfcal(Qcf,cases)
	   ...
	   P = (aindex - 1.)*(Qcf(en) - 0.5*dn*(vx**2 + vy**2 + vz**2))
           ...
           P = (Qcf(exx) + Qcf(eyy) + Qcf(ezz) - dn*(vx**2 + vy**2 + vz**2))/3.
        end if
