...
!-------------------------------------------------------------------------------
    subroutine get_region_Sflux(Sflux_out, dt)
        implicit none
        real, dimension(nface,1:nx1,1:ny1,1:nz1,3,3), intent(out) :: Sflux_out
        ...
        do i=1,nx1
            do ipnt=1,npts_llns
                ...
                Sflux_out(ipnt,i,j,k,1,2) = eta_d*Gxy
                Sflux_out(ipnt,i,j,k,1,3) = eta_d*Gxz
                Sflux_out(ipnt,i,j,k,2,3) = eta_d*Gyz
                Sflux_out(ipnt,i,j,k,2,1) = Sflux_out(ipnt,i,j,k,1,2)
                Sflux_out(ipnt,i,j,k,3,1) = Sflux_out(ipnt,i,j,k,1,3)
                Sflux_out(ipnt,i,j,k,3,2) = Sflux_out(ipnt,i,j,k,2,3)

                trG   = (Gxx + Gyy + Gzz)
                trGd3 = c1d3*trG
                trG_zeta = zeta_d*trG

                Sflux_out(ipnt,i,j,k,1,1) = eta_d*(Gxx - trGd3) + trG_zeta
                Sflux_out(ipnt,i,j,k,2,2) = eta_d*(Gyy - trGd3) + trG_zeta
                Sflux_out(ipnt,i,j,k,3,3) = eta_d*(Gzz - trGd3) + trG_zeta
            enddo
        enddo
        enddo
        enddo
